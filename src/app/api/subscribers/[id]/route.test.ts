...
const { prismaMock, authMock } = vi.hoisted(() => ({
  prismaMock: { subscriber: { delete: vi.fn() } },
...
beforeEach(() => {
  ...
  prismaMock.subscriber.delete.mockResolvedValue({ id: 7 });
});
 ...
 * - `parseInt` truncating `1abc` onto subscriber 1, `Number()` resolving `0x1f`
 * onto subscriber 31, and a value past the column's 32-bit range - and what they
 * prove here is that this route actually calls the shared schema. This one
 * deletes a mailing-list record, which nothing here can put back.
 */
...
describe("DELETE /api/subscribers/[id] path id", () => {
  it.each(REJECTED_IDS)(
    ...
    async (_label, id) => {
      ...
      expect(prismaMock.subscriber.delete).not.toHaveBeenCalled();
    },
  );
