...
import RecipesArchivePage from "./page";
import { ARCHIVE_PAGE_SIZE } from "@/utils/contentArchive";
import { MAX_SEARCH_LENGTH } from "@/utils/pagination";

// NOTE ON COVERAGE SCOPE
// ----------------------
// `fetchArchiveRecipes` is module-private and deliberately stays that way. It is
...
// server, so the whole tree — imagery included — renders here: a signed recipe
// emits a real <img> under static render, which is exactly what ships in the SSR
// HTML. What static render cannot show is the search control's interactive
// behavior (debounce, navigation); that gap is documented and covered as far as
// it can be in ContentArchiveSearch.test.tsx.
//
// The seam this file exists to pin is the URL contract. `?page=` and `?q=` are
// user input; they are parsed, clamped, translated into the API's own spelling
// (`q` in the address bar becomes `search` on the wire) and re-encoded on the
// way back out. Each of those is asserted against the live page rather than
...
function recipe(id: number, title: string): RecipePayload {
  return {
    id,
    title,
    author: `Author ${id}`,
    description: `Summary ${id}`,
    coverPhoto: `recipes/${id}.jpg`,
 ...
 * catch.
 */
const NO_CONTENT_COPY = "any recipes yet. Check back soon";
...
/** Stubs the recipes endpoint with a list and the archive-wide total. */
function stubRecipes(recipes: RecipePayload[], total = recipes.length): void {
  stubFetch({ ok: true, json: async () => ({ recipes, total }) });
}
...
async function renderPage(
  ...
  return renderToStaticMarkup(
    await RecipesArchivePage({ searchParams: Promise.resolve(params) }),
  );
}
...
describe("RecipesArchivePage happy path", () => {
  it("renders one row per recipe, under the archive heading", async () => {
    stubRecipes(FULL_PAGE, 47);

    const html = await renderPage();

    expect(countTags(html, "h2")).toBe(ARCHIVE_PAGE_SIZE);
    expect(html).toContain(">All Recipes<");
    expect(html).toContain("Recipe 1");
    expect(html).toContain(`Recipe ${ARCHIVE_PAGE_SIZE}`);
    expect(html).toContain('href="/recipes/1"');
    expect(html).toContain('aria-label="View recipe: Recipe 1"');
  });
  ...
  it("requests the first page at the shared archive page size, uncached", async () => {
    ...
    expect(calls[0].url).toContain(
      `/api/recipes?page=1&pageSize=${ARCHIVE_PAGE_SIZE}`,
    );
  ...
  it("offers a way back to the landing page", async () => {
    ...
    expect(await renderPage()).toContain('href="/recipes-landing"');
  });
});

describe("RecipesArchivePage result count", () => {
  it("counts the whole archive, not the rows on this page", async () => {
    ...
    // "12 recipes" above a four-page pager, which is simply false. The number
    // has to be the API's `total`.
    stubRecipes(FULL_PAGE, 47);

    const html = await renderPage();

    expect(html).toContain(">47 recipes<");
    expect(html).not.toContain(`>${ARCHIVE_PAGE_SIZE} recipes<`);
  ...
  it("counts the filtered archive when a search is active", async () => {
    // `total` already reports the filtered count, and the filtered count is the
    // honest one to show because it is what the pager is paging through.
    stubRecipes([recipe(1, "Gin Fizz"), recipe(2, "Gin Rickey")], 2);

    const html = await renderPage({ q: "gin" });

    expect(html).toContain(`2 recipes matching “gin”`);
  });
  ...
  it("states no count when the API omits a total", async () => {
    ...
    stubFetch({ ok: true, json: async () => ({ recipes: FULL_PAGE }) });
...
describe("RecipesArchivePage URL contract", () => {
  it("sends the requested page through to the API", async () => {
    ...
    expect(await renderPage({ page: "3" })).toContain("Page 3 of 4");
  });
  ...
  it("translates the URL's `q` into the API's `search`", async () => {
    ...
    // /api/recipes spells the same thing `search`. The translation happens on
    // this page and nowhere else, so nothing else can pin it.
    stubRecipes([recipe(1, "Gin Fizz")], 1);
  ...
  it("sends no search param for an unfiltered archive", async () => {
    // Not `search=`: an empty term must disable the filter rather than being
    // sent as a search for the empty string, which would also key the route's
    // cache differently.
    stubRecipes(FULL_PAGE, 47);
  ...
  it("trims the term before searching and before showing it", async () => {
    ...
    expect(searchParamSent()).toBe("gin");
    expect(html).toContain(`1 recipe matching “gin”`);
  });
  ...
  it("searches and displays the same bounded term for an over-long query", async () => {
    ...
    // was asked to filter on. Bounding at only one of the two sites would search
    // 100 characters and claim to have searched 150, or the reverse.
    ...
    // The identical string reaches the wire and the page.
  ...
  it("carries the search term across a page change", async () => {
    // The pager's hrefs are what make a filtered page shareable; dropping the
    // term on page 2 would silently widen the result set under the reader.
    stubRecipes(FULL_PAGE, 47);

    const html = await renderPage({ q: "gin", page: "2" });

    expect(html).toContain('href="/recipes?q=gin"');
    expect(html).toContain('href="/recipes?q=gin&amp;page=3"');
  });
  ...
  it("re-encodes a term that would otherwise break out of its slot", async () => {
    ...
    // URLSearchParams encoding, never concatenation.
    expect(html).toContain("q=gin+%26+tonic");
    expect(html).not.toContain("q=gin & tonic");
    // And it round-trips back to the term the reader typed, which is the
    // property that matters: the encoding is only correct if it decodes.
  ...
  it("escapes a term on its way into the page rather than trusting it", async () => {
    // `?q=` is user input rendered into the header and the empty message. This
    // is the assertion that it arrives as text, not as markup.
    stubRecipes([], 0);
...
describe("RecipesArchivePage empty states", () => {
  ...
  it("says the search matched nothing, never that the archive is empty", async () => {
    // A user who searched "gin" and got nothing must be told the search matched
    // nothing. Being told the bar is empty, when it holds 47 recipes, is the
    // failure the ordering inside archiveEmptyState prevents.
    stubRecipes([], 0);
    ...
    expect(html).toContain(`0 recipes matching “gin”`);
    expect(html).toContain(
      "No recipes match that search. Try a shorter one, or clear it to see the whole bar.",
    );
  ...
  it("reports a page past the end, and clamps the pager back into range", async () => {
    // ?page=999 must not strand the reader on a dead page: the pager reports a
    // page that exists, so there is still a way back.
    stubRecipes([], 47);

    const html = await renderPage({ page: "999" });

    expect(countTags(html, "h2")).toBe(0);
    expect(html).toContain("That page is past the end of the bar.");
    expect(html).toContain("Page 4 of 4");
    expect(html).toContain('href="/recipes?page=3"');
  });
});

describe("RecipesArchivePage failure contract", () => {
  ...
  it("degrades to the empty state on a non-ok response, without parsing it", async () => {
    const json = vi.fn(async () => ({ recipes: FULL_PAGE, total: 47 }));
  ...
  it("degrades to the empty state when the payload has no recipes key", async () => {
    stubFetch({ ok: true, json: async () => ({}) });

    expectUsableEmptyPage(await renderPage());
  });

  it("degrades to the empty state when recipes is null", async () => {
...
describe("RecipesArchivePage server-rendered imagery", () => {
  ...
  it("never sends a missing cover photo to the signer", async () => {
    stubRecipes(
      [recipe(1, "Signed"), { ...recipe(2, "Unsigned"), coverPhoto: null }],
      2,
    );
  ...
  it("emits a real <img> per signed cover photo in the server response", async () => {
    ...
    expect(html).toContain(encodeURIComponent(signedUrlFor("recipes/1.jpg")));
  ...
  it("keeps every row when signing is unavailable", async () => {
    ...
    // parse src" and fails the whole render. The page must fall back to the
    // client-resolved path instead.
    stubRecipes(FULL_PAGE, 47);
